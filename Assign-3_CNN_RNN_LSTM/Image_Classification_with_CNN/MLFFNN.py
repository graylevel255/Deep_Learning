...
X_train_file = 'train_x_VGG.txt'
X_val_file = 'val_x_VGG.txt'
X_test_file = 'test_x_VGG.txt'
Y_train_file = 'train_Y_VGG.txt'
Y_val_file = 'val_Y_VGG.txt'
Y_test_file = 'test_Y_VGG.txt'
...
# Network Parameters
n_hidden_1 = 786  # 1st layer number of neurons
n_hidden_2 = 128    #t 2nd layer number of neurons
...
keep_prob = tf.constant(.6)
epsilon = 1e-3
beta =0.01

# Store layers weight & bias
weights = {
    'h1':tf.get_variable(shape=[num_input, n_hidden_1],initializer=tf.contrib.layers.xavier_initializer(seed = 1)),
    'h2':tf.get_variable( shape=[n_hidden_1, n_hidden_2],initializer=tf.contrib.layers.xavier_initializer(seed = 2)),
    'out':tf.get_variable(shape=[n_hidden_2,num_classes], initializer=tf.contrib.layers.xavier_initializer(seed = 3))
}
biases = {
...
def neural_net(x, keep_prob):
    ...
    out_layer = tf.nn.sigmoid(tf.matmul(drop_out_2, weights['out']) + biases['out'])
...
with tf.Session() as sess:
    ...
    with open('train_predicted.txt', 'w') as f:
