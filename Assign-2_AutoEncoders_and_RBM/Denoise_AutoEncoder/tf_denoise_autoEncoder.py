...
weights = {
    ...
    'encoder_h2': tf.Variable(tf.random_normal([h1, h2], seed = 2)*0.01),
    'encoder_h3': tf.Variable(tf.random_normal([h2, h3], seed = 5)*0.01),
    'decoder_h1': tf.Variable(tf.random_normal([h3, h2], seed = 3)*0.01),
    'decoder_h2': tf.Variable(tf.random_normal([h2, h1], seed = 4)*0.01),
    'decoder_h3': tf.Variable(tf.random_normal([h1, ip_dim], seed = 6)*0.01),

}

biases = {
    'encoder_b1': tf.Variable(tf.random_normal([h1], seed = 7)*0.01),
    'encoder_b2': tf.Variable(tf.random_normal([h2], seed = 8)*0.01),
    'encoder_b3': tf.Variable(tf.random_normal([h3], seed=9) * 0.01),
    'decoder_b1': tf.Variable(tf.random_normal([h2], seed = 10)*0.01),
    'decoder_b2': tf.Variable(tf.random_normal([h1], seed = 11) * 0.01),
    'decoder_b3': tf.Variable(tf.random_normal([ip_dim], seed = 12)*0.01),
}
...
def encoder(x):
    ...
    enc_layer2 = tf.nn.sigmoid(tf.add(tf.matmul(enc_layer1, weights['encoder_h2']), biases['encoder_b2']))
    enc_layer3 = tf.nn.sigmoid(tf.add(tf.matmul(enc_layer2, weights['encoder_h3']), biases['encoder_b3']))
...
def decoder(x):
    ...
    dec_layer2 = tf.nn.sigmoid(tf.add(tf.matmul(dec_layer1, weights['decoder_h2']), biases['decoder_b2']))
    dec_layer3 = tf.nn.sigmoid(tf.add(tf.matmul(dec_layer2, weights['decoder_h3']), biases['decoder_b3']))
...
with tf.Session() as sess:
    ...
    # for i in range(n):
    #         # MNIST test set
    #     batch_test = next_batch_test(i)
    #     batch_y = next_batch_ytrue_test(i)
    #         # Encode and decode the digit image
    #
    #     g_test = sess.run(decoder_op, feed_dict={ip_data: batch_test})
    #         # g = g.astype(int)
    #     g_test = g_test.round()
    #
    #          # Display noisy images
    ...
    #         batch_test[j].reshape([28, 28])
    #
    #         # Display reconstructed images
    #     for j in range(n):
    #             # Draw the reconstructed digits
    #         canvas_recon[i * 28:(i + 1) * 28, j * 28:(j + 1) * 28] = \
    #         g_test[j].reshape([28, 28])
    #
