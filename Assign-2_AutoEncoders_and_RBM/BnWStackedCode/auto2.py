...
with open('./ip2ae2.csv', 'r') as csvfile:
    ip2ae2 = list(csv.reader(csvfile))
features = np.array(ip2ae2)
...
# ******************** BUILDING THE MODEL ******************** #
n_neurons1 = config.ae2h1
n_neurons2 = config.ae2h2
...
with tf.Session() as sess:
    ...
    for i in range(config.ae2epoch):
        ...
        epochs += 1
    ip2ae3 = sess.run(l2, feed_dict={x: features})
    w2_right = sess.run(W1)
    b2_right = sess.run(b1)
    w3_left = sess.run(W2)
    b3_left = sess.run(b2)
    with open('ip2ae3.csv', 'w') as writeFile:
        writer = csv.writer(writeFile)
        writer.writerows(ip2ae3)
    writeFile.close()
    with open('w2_right.csv', 'w') as writeFile:
        writer = csv.writer(writeFile)
        writer.writerows(w2_right)
    writeFile.close()

    with open('b2_right.csv', 'w') as writeFile:
        writer = csv.writer(writeFile)
        writer.writerow(b2_right)
    writeFile.close()

    with open('w3_left.csv', 'w') as writeFile:
        writer = csv.writer(writeFile)
        writer.writerows(w3_left)
    writeFile.close()

    with open('b3_left.csv', 'w') as writeFile:
        writer = csv.writer(writeFile)
        writer.writerow(b3_left)
    writeFile.close()
print("OPTIMIZATION OF AE2 FINISHED")