...
# Network Parameters
num_hidden_1 = 300     # nodes in 1st Hidden Layer
num_hidden_2 = 150     # nodes in 2nd Hidden Layer
num_linear = 150       # nodes in linear layer (reduced Representation)
...
weights = {
    ...
    'encoder_lin_w': tf.Variable(tf.truncated_normal([num_hidden_2, num_linear], seed=3)*.01),
    'decoder_h1': tf.Variable(tf.truncated_normal([num_linear, num_hidden_2], seed=4)*.01),
    'decoder_h2': tf.Variable(tf.random_normal([num_hidden_2, num_hidden_1], seed=5)*.01),
    'decoder_o': tf.Variable(tf.random_normal([num_hidden_1, num_input], seed=6)*.01)

}
biases = {
    'encoder_b1': tf.Variable(tf.random_normal([num_hidden_1], seed=7)*.01),
    'encoder_b2': tf.Variable(tf.random_normal([num_hidden_2], seed=8)*.01),
    'linear_b': tf.Variable(tf.random_normal([num_linear], seed=9)*.01),
    'decoder_b1': tf.Variable(tf.random_normal([num_hidden_2], seed=10)*.01),
    'decoder_b2': tf.Variable(tf.random_normal([num_hidden_1], seed=11)*.01),
    'decoder_o': tf.Variable(tf.random_normal([num_input], seed=12)*.01)
...
def encoder(x):
    ...
    linear_layer = tf.add(tf.matmul(layer_2, weights['encoder_lin_w']), biases['linear_b'])
...
def decoder(x):
    ...
    output = tf.add(tf.matmul(layer_2, weights['decoder_o']), biases['decoder_o'])
