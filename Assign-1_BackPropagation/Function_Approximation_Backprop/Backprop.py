...
class NeuralNet:
    def __init__(self, layer_nodes, loss=af.SSE, activation_func=af.Sigmoid):
        self.layer_nodes = layer_nodes
        self.layer_count = len(layer_nodes)
        self.loss = loss
        self.activation_func = activation_func
        # np.random.seed(17)
        self.bias = [0.0*np.random.randn(i) for i in layer_nodes[1:]]
    ...
    def __get_activation__(self, present_layer, current_activations):
        get_activation = np.dot(self.weights[present_layer], current_activations) + self.bias[present_layer]
    ...
    def __forward_pass__(self, incoming_layer):
        ...
        incoming_layer = self.__get_activation__(self.layer_count-2, incoming_layer)
    ...
    def __back_propagation__(self, x, y):
        ...
        activation_derivatives.append(np.ones(self.layer_nodes[self.layer_count-1]))
    ...
    def update_weights_batch(self, x, y, N, eta, update_rule="adam", alpha=0.9, epsilon=1e-8):
    ...
    def update_weights_stochastic(self, x, y, N, eta, update_rule="ada_grad", alpha = .9, epsilon = 1e-6):
        # N : size of training data
        # x : Input Data (features)
        # y : desired output
    ...
    def test(self, x, y):
        ...
        error = .5 * np.sum(np.sum(diff, axis=0))/ n
