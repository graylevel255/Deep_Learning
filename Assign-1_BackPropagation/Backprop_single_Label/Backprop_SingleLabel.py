...
class NeuralNet:
    def __init__(self, layer_nodes, loss=af.crossEntropy, activation_func=af.SoftPlus):
        ...
        #np.random.seed(77)
        np.random.seed(68)
        self.bias = [0.01 * np.random.randn(i) for i in layer_nodes[1:]]
        #np.random.seed(39)
        np.random.seed(34)
        pairs = zip(layer_nodes[:self.layer_count-1], layer_nodes[1:])
        self.weights = [.01 * np.random.randn(p, c)*np.sqrt(2.0/p) for c, p in pairs]
    ...
    def __forward_pass__(self, incoming_layer):
        ...
        incoming_layer = af.Softmax.activ_fun(self.__get_activation__(self.layer_count-2, incoming_layer), self.layer_count-2)
        return incoming_layer

    # x is the input to the neural network and y is the expected output
    def __back_propagation__(self, x, y, label):
        temp_activation = x
        ...
        temp2_matrix.append(self.__get_activation__(self.layer_count-2, temp_activation))
        temp_activation = af.Softmax.activ_fun(self.__get_activation__(self.layer_count-2, activation_values[self.layer_count-2]), self.layer_count-1 , beta, delta)
        activation_values.append(temp_activation)
        activation_derivatives.append(self.loss.activ_fun_der(temp_activation, self.layer_count-1, beta, delta))
        ...
        error = self.loss.delta(activation_values[self.layer_count - 1], y)  # (estimated-expected)()
        ...
        for l in reversed(range(1, self.layer_count - 1)):
            aux1 = (self.weights[self.layer_count-2][int(label)]) - np.dot(np.array(self.weights[self.layer_count-2].transpose()), activation_values[self.layer_count-1])
            aux2 = -np.array(aux1) * np.array(activation_derivatives[l])
            bias_gradient[l-1] = aux2
            # x = x.reshape((x.shape[0], 1))
            activation_values[l - 1] = activation_values[l-1].reshape((1, activation_values[l-1].shape[0]))
            aux2 = aux2.reshape((1, aux2.shape[0]))
            weight_gradient[l-1] = beta * np.dot(aux2.transpose(), activation_values[l-1])
            return bias_gradient, weight_gradient, activation_derivatives


    def update_weights_batch(self, x, y, label,  N, eta, update_rule="delta", alpha=0.9, epsilon=1e-8):
        ...
        for i in range(N):
            curr_grad_bias, curr_grad_weights, sh = self.__back_propagation__(x[i], y[i], label[i][0])
        ...
        if update_rule == "delta":
            ...
            self.weights = [w_old - eta * grad_w + alpha * del_w for w_old, grad_w, del_w in zip(self.weights, grad_weights, self.delta_weights)]
            self.bias = [b_old - eta * grad_b + alpha * del_b for b_old, grad_b, del_b in zip(self.bias, grad_bias, self.delta_bias)]
            ...
            self.weights = [w_old - (eta * np.array(grad_w, dtype=np.float))/(np.sqrt(epsilon + np.array(r2_w, dtype=np.float)))
                                for w_old, grad_w, r2_w in zip(self.weights, grad_weights, self.delta_weights)]
            self.bias = [b_old - (eta * np.array(grad_b, dtype=np.float))/(np.sqrt(epsilon + np.array(r2_b, dtype=np.float)))
                         for b_old, grad_b, r2_b in zip(self.bias, grad_bias, self.delta_bias)]

        elif update_rule == "rmsProp":
            rho = 0.95
            self.delta_weights = [rho * r2_w + (1-rho)*(np.power(g_w, 2)) for r2_w, g_w in zip(self.delta_weights, grad_weights)]
            self.delta_bias = [rho * r2_b + (1-rho)*(np.power(g_b, 2)) for r2_b, g_b in zip(self.delta_bias, grad_bias)]
            ...
            self.delta_weights = [rho1 * r2_w + (1-rho1)*(np.power(g_w, 2)) for r2_w, g_w in zip(self.delta_weights, grad_weights)]
            self.delta_bias = [rho1 * r2_b + (1-rho1)*(np.power(g_b, 2)) for r2_b, g_b in zip(self.delta_bias, grad_bias)]

            self.weights = [
                w_old - ((np.sqrt(np.array(U_w, dtype=np.float)+epsilon)) * np.array(grad_w, dtype=np.float)) / (np.sqrt(epsilon + np.array(r2_w, dtype=np.float)))
                for w_old, U_w, grad_w, r2_w in zip(self.weights, self.U_weights, grad_weights, self.delta_weights)]
            self.bias = [
                b_old - ((np.sqrt(np.array(U_b, dtype=np.float) + epsilon)) * np.array(grad_b, dtype=np.float)) / (np.sqrt(epsilon + np.array(r2_b, dtype=np.float)))
                for b_old, U_b, grad_b, r2_b in zip(self.bias, self.U_bias, grad_bias, self.delta_bias)]

            # Update U for the next iteration
            dW = [((np.sqrt(np.array(U_w, dtype=np.float) + epsilon)) * np.array(grad_w, dtype=np.float)) / (np.sqrt(epsilon + np.array(r2_w, dtype=np.float)))
                  for U_w, grad_w, r2_w in zip(self.U_weights, grad_weights, self.delta_weights)]
            dB = [((np.sqrt(np.array(U_b, dtype=np.float) + epsilon)) * np.array(grad_b, dtype=np.float)) / (np.sqrt(epsilon + np.array(r2_b, dtype=np.float)))
            ...
            self.U_weights = [rho2 * v_w + (1 - rho2) * (np.power(g_w, 2)) for v_w, g_w in zip(self.U_weights, grad_weights)]
            ...
            self.weights = [
                w_old - ((eta * np.array(u_hat_w, dtype=np.float)) / (np.sqrt(epsilon + np.array(v_hat_w, dtype=np.float))))
                for w_old, u_hat_w, v_hat_w in zip(self.weights, self.delta_weights_hat, self.U_weights_hat)]
            self.bias = [
                b_old - ((eta * np.array(u_hat_b, dtype=np.float)) / (np.sqrt(epsilon + np.array(v_hat_b, dtype=np.float))))
    ...
    def update_weights_stochastic(self, x, y, label, N, eta, update_rule="delta", alpha = .9, epsilon = 1e-6):
        # N : size of training data
        # x : Input Data (features)
        # y : desired output
        ...
        for i in range(N):
            curr_grad_bias, curr_grad_weights, sh2 = self.__back_propagation__(x[i], y[i], label[i][0])
            self.epoch_num += 1
            # update the weights and bias
            if update_rule == "delta":
                ...
                # updating the parameters using the learning rate for each parameter  by adagrad
                #  updating r for next iteration, i.e. accumulating squares of gradients for next iteration
                self.delta_weights = [r2_w + (np.power(g_w, 2)) for r2_w, g_w in zip(self.delta_weights, curr_grad_weights)]
                ...
                # rms values used in denominator same as Ada Grad
                self.delta_weights = [rho1 * r2_w + (1 - rho1) * (np.power(g_w, 2)) for r2_w, g_w in zip(self.delta_weights, curr_grad_weights)]
                self.delta_bias = [rho1 * r2_b + (1 - rho1) * (np.power(g_b, 2)) for r2_b, g_b in zip(self.delta_bias, curr_grad_bias)]
                ...
                self.weights = [
                    ...
                    for w_old, U_w, grad_w, r2_w in zip(self.weights, self.U_weights, curr_grad_weights, self.delta_weights)]
                ...
                # u = sum of values for g(w)
                self.delta_weights = [rho1 * u_w + (1 - rho1) * g_w for u_w, g_w in zip(self.delta_weights, curr_grad_weights)]
                self.delta_bias = [rho1 * u_b + (1 - rho1) * g_b for u_b, g_b in zip(self.delta_bias, curr_grad_bias)]

                # v = sum of values of g(w)^2
                self.U_weights = [rho2 * v_w + (1 - rho2) * (np.power(g_w, 2)) for v_w, g_w in zip(self.U_weights, curr_grad_weights)]
                self.U_bias = [rho2 * v_b + (1 - rho2) * (np.power(g_b, 2)) for v_b, g_b in zip(self.U_bias, curr_grad_bias)]
    ...
    def test(self, x, y):
        ...
        for i in range(n):
            ...
            # loss += np.dot(y[i], np.log(predicted_y[i]))
        # predicted_y = [list(i) for i in zip(*predicted_y)]
        for i in range(n):
            loss += np.dot(y[i], np.log(predicted_y[i]))
        return predicted_y, -1.0*loss/n
